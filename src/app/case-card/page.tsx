...
function StepRow({
  ...
  return (
    <li className="border-l-[3px] border-[var(--marigold)] py-2 pl-4">
      ...
      {detail !== title && (
        <span className="mt-1 block text-sm leading-relaxed text-[#536059]">
          {detail}
        </span>
      )}
      {node.link && (
        <a
          className="mt-1 block break-all text-sm font-bold text-[var(--green)]"
          href={node.link.url}
          target="_blank"
      ...
      {note && (
        <em className="mt-1 block text-sm font-bold not-italic text-[#7c2b21]">
          {note}
        </em>
      )}
    </li>
...
export default function CaseCardPage({
  ...
  const [ownedJurisdiction, setOwnedJurisdiction] =
    useState<WorkflowJurisdiction | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/cases/${encodeURIComponent(caseIdParam)}`)
      ...
      .then(({ case: savedCase, definition, trust, jurisdiction }) => {
        ...
        registerWorkflowDefinition(
          definition,
          savedCase.snapshot.workflowVersionId
        );
  ...
  useEffect(() => {
    ...
    fetch(`/api/cases/${encodeURIComponent(caseIdParam)}/outcomes`)
      .then(async (response) => {
        const result = await response.json();
        if (!response.ok)
          throw new Error(result.error || "outcomes unavailable");
        return result;
      })
      .then(({ outcomes: list }) => {
  ...
  if (!caseIdParam) {
    return (
      <main className="mx-auto min-h-screen w-full max-w-3xl px-4 py-6">
        <a
          className="inline-flex min-h-11 items-center rounded-xl bg-[#eee5d8] px-4 text-sm font-extrabold text-[var(--green)] no-underline"
          href="/"
        >
          {t("caseCard.back")}
        </a>
  ...
  if (caseLoadFailed) {
    return (
      <main className="mx-auto min-h-screen w-full max-w-3xl px-4 py-6">
        ...
        <p
          className="mt-4 rounded-xl border border-[#d9a79f] bg-[#fbeceb] p-4 font-bold text-[#7c2b21]"
          role="alert"
        >
  ...
  const stateById = new Map(
    snapshot.nodes.map((entry) => [entry.id, entry.state])
  );
  ...
  const cleared = steps.filter((step) =>
    isClearedBlocker(snapshot, step.node.id)
  );
  const complete =
    steps.every((step) => step.state === "done" || step.state === "pending") &&
    steps.some(
      (step) => step.node.type === "case-complete" && step.state === "done"
    );
  const currentVisit = active?.node.visit;
  const referenceNumbers = (snapshot.reports ?? []).flatMap(
    ({ referenceNumber }) => (referenceNumber ? [referenceNumber] : [])
  );
  const grievanceNeedsPreparation =
    snapshot.artifacts.includes("escalation-draft") &&
  ...
  const ui =
    locale === "hi"
      ? {
          ...
          noDocuments:
            "इस केस में कोई तैयार दस्तावेज़ नहीं है। दर्ज जवाब और इतिहास नीचे हैं।",
          download: "Word दस्तावेज़ डाउनलोड करें",
          prepareGrievance: "अपनी शिकायत तैयार करें",
          prepareGrievanceDetail:
            "अपने दर्ज केस विवरण से AI मसौदा बनाएँ, फिर उसे पढ़कर सुधारें और डाउनलोड करें।",
      ...
      : {
          ...
          historyHint:
            "Completed steps, setbacks, recorded responses and outcomes",
          ...
          noDocuments:
            "This case has no generated documents. Recorded responses and history remain below.",
          download: "Download Word document",
          prepareGrievance: "Prepare your grievance",
          prepareGrievanceDetail:
            "Use your recorded case details to create an AI draft, then review, edit and download it.",
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-3xl px-4 py-5 sm:px-6 sm:py-8">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 print:hidden">
        <a
          className="inline-flex min-h-11 items-center rounded-xl border border-[var(--line)] bg-[#eee5d8] px-4 text-sm font-extrabold text-[var(--green)] no-underline focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[var(--green)]"
          href={`/?caseId=${encodeURIComponent(caseIdParam)}`}
        >
          {t("caseCard.continue")}
        </a>
        <div className="flex flex-wrap items-center justify-end gap-2">
          <LanguageSwitcher />
          <button
            className="min-h-11 rounded-xl border-0 bg-[var(--marigold)] px-4 text-sm font-extrabold text-[#2f250f] focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[var(--green)]"
            type="button"
      ...
      <article className="overflow-hidden rounded-3xl border border-[var(--line)] bg-[#fffdf9] shadow-[0_14px_40px_rgba(52,43,27,0.07)] print:border-0 print:shadow-none">
        <header className="border-b border-[var(--line)] px-5 py-6 sm:px-7">
          ...
          <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
            <div>
              <h1 className="m-0 text-2xl font-extrabold leading-tight">
                {translate(seed.title, locale)}
              </h1>
              <p className="mt-1 text-sm leading-relaxed text-[#536059]">
                {translate(seed.subtitle, locale)}
              </p>
            </div>
          ...
          <dl className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3">
            <div className="rounded-xl bg-[#f2f5f2] p-3">
              <dt className="text-[.68rem] font-extrabold uppercase tracking-wide text-[#627069]">
                {ui.status}
              </dt>
            ...
            <div className="rounded-xl bg-[#f2f5f2] p-3">
              ...
              <dd className="mt-1 text-sm font-extrabold text-[var(--ink)]">
                {done.filter((step) => step.node.id !== "case-done").length} /{" "}
                {steps.filter((step) => step.node.id !== "case-done").length}
              </dd>
            </div>
            <div className="col-span-2 rounded-xl bg-[#f2f5f2] p-3 sm:col-span-1">
              <dt className="text-[.68rem] font-extrabold uppercase tracking-wide text-[#627069]">
                {ui.documents}
              </dt>
              <dd className="mt-1 text-sm font-extrabold text-[var(--ink)]">
          ...
          <div className="mt-3">
            {ownedTrust && ownedJurisdiction && (
              <TrustDisclosure
                trust={ownedTrust}
                jurisdiction={ownedJurisdiction}
                locale={locale}
              />
            )}
          </div>
        ...
        <div className="grid gap-7 px-5 py-6 sm:px-7">
          <section aria-labelledby="next-action-title">
            ...
            {grievanceNeedsPreparation ? (
              <div className="mt-2 rounded-2xl border-l-4 border-[var(--green)] bg-[#eef5f0] p-4 sm:p-5">
                <h2
                ...
                <p className="mt-2 text-sm leading-relaxed text-[#45534c]">
                  {ui.prepareGrievanceDetail}
                </p>
                <a
                  className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-[var(--green)] px-4 text-sm font-extrabold text-white no-underline print:hidden"
                  href="#artifact-escalation-draft"
                >
                  {ui.prepareGrievanceAction} →
                </a>
              </div>
            ) : active ? (
              <div className="mt-2 rounded-2xl border-l-4 border-[var(--green)] bg-[#eef5f0] p-4 sm:p-5">
                <h2
                  ...
                  id="next-action-title"
                >
                  {translate(active.node.title, locale)}
                </h2>
                <p className="mt-2 text-sm leading-relaxed text-[#45534c]">
                  {translate(active.node.detail, locale)}
                </p>
                <p className="mt-2 text-sm font-extrabold text-[#7c2b21]">
                  {active.state === "verifying"
                    ? t("caseCard.current.verifying")
                    : translate(active.node.ask, locale)}
                </p>
                <a
                  className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-[var(--green)] px-4 text-sm font-extrabold text-white no-underline print:hidden"
                  href={`/?caseId=${encodeURIComponent(caseIdParam)}`}
              ...
              <p className="mt-2 text-sm text-[#536059]">
                {t("caseCard.current.empty")}
              </p>
            )}
          </section>
          ...
          {currentVisit && (
            ...
            >
              ...
              <h2 className="mt-1 text-lg font-extrabold" id="visit-title">
                {translate(currentVisit.office, locale)}
              </h2>
              <p className="mt-1 text-sm leading-relaxed text-[#536059]">
                {translate(currentVisit.why, locale)}
              </p>
              <dl className="mt-4 grid gap-3 text-sm">
                <div>
                  <dt className="text-xs font-extrabold uppercase tracking-wide text-[var(--green)]">
                    {t("caseCard.visits.carry")}
                  </dt>
                  <dd className="mt-1 leading-relaxed">
                    {tList(currentVisit.carry, locale).join(" · ")}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs font-extrabold uppercase tracking-wide text-[var(--green)]">
                    {t("caseCard.visits.script")}
                  </dt>
                  <dd className="mt-1 rounded-xl bg-[#fff8e8] p-3 font-bold leading-relaxed">
                    “{translate(currentVisit.script, locale)}”
                  </dd>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <dt className="text-xs font-extrabold uppercase tracking-wide text-[var(--green)]">
                      {t("caseCard.visits.expect")}
                    </dt>
                    <dd className="mt-1">
                      {translate(currentVisit.expect, locale)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs font-extrabold uppercase tracking-wide text-[var(--green)]">
                      {t("caseCard.visits.collect")}
                    </dt>
                    <dd className="mt-1">
                      {translate(currentVisit.collect, locale)}
                    </dd>
                  </div>
                </div>
              </dl>
              <p className="mb-0 mt-4 text-xs font-bold text-[#735c37]">
                {t("caseCard.visits.noAgent")}
              </p>
            </section>
          )}
              ...
              snapshot.artifacts.map((id) => {
                ...
                return (
                  <div
                    key={id}
                    id={`artifact-${id}`}
                    className="mt-3 scroll-mt-4 rounded-2xl border border-[var(--line)] bg-white p-4 sm:p-5"
                  >
                    <h3 className="m-0 text-base font-extrabold">
                      {translate(artifact.title, locale)}
                    </h3>
                    <p className="mt-1 text-sm leading-relaxed text-[#536059]">
                      {translate(artifact.subtitle, locale)}
                    </p>
                    {id === "escalation-draft" ? (
                      <ArtifactDraftPanel
                        caseId={caseIdParam}
                        ...
                        locale={locale}
                        onDraftChange={(draft) =>
                          setOwnedSnapshot((currentSnapshot) =>
                            currentSnapshot
                              ? {
                                  ...currentSnapshot,
                                  artifactDrafts: {
                      ...
                      <>
                        <ul className="mt-3 grid gap-1 pl-5 text-sm leading-relaxed">
                          {renderArtifactBody(id, snapshot, locale).map(
                            (line, index) => (
                        ...
                        <a
                          className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-[var(--green)] px-4 text-sm font-extrabold text-white no-underline print:hidden"
                          href={`/api/cases/${encodeURIComponent(caseIdParam)}/artifacts/${encodeURIComponent(id)}?locale=${locale}`}
                        >
                          {ui.download}
                        </a>
                      </>
                    )}
                    <p className="mb-0 mt-3 text-xs font-bold text-[#7a530a]">
                      {t("caseCard.artifacts.draftLabel")}
                    </p>
                  </div>
                );
              })
            ) : (
              <p className="mt-2 text-sm leading-relaxed text-[#536059]">
                {ui.noDocuments}
              </p>
            )}
          </Section>
          ...
          <details className="rounded-2xl border border-[var(--line)] bg-white print:border-0">
            <summary className="cursor-pointer p-4 focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[var(--green)] sm:p-5 print:hidden">
              <span className="ml-2 inline-block align-middle">
                <strong className="block text-base">{ui.history}</strong>
                <span className="mt-1 block text-sm text-[#536059]">
                  {ui.historyHint}
                </span>
              </span>
            </summary>
            <div className="grid gap-6 border-t border-[var(--line)] p-4 sm:p-5 print:!grid print:border-0">
              <div>
                <h3 className="m-0 text-sm font-extrabold">
                  {t("caseCard.completed.title")}
                </h3>
                {done.length > 0 ? (
                  <ol className="mt-2 grid gap-1 p-0">
                    {done.map((step) => (
                      <StepRow
                        key={step.node.id}
                        node={step.node}
              ...
              <div>
                <h3 className="m-0 text-sm font-extrabold">{ui.responses}</h3>
                {(snapshot.reports ?? []).length > 0 ? (
                  <ol className="mt-2 grid gap-2 p-0">
                    {(snapshot.reports ?? []).map((report, index) => {
                      const step = seed.nodes.find(
                        ({ id }) => id === report.stepId
                      );
                      return (
                        <li
                          className="border-l-[3px] border-[var(--marigold)] py-2 pl-4 text-sm"
                          key={`${report.recordedAt}-${index}`}
                        >
                          ...
                          <span className="mt-1 block text-[#536059]">
                            {locale === "hi"
                              ? `जवाब की तारीख: ${report.responseDate}`
                              : `Response date: ${report.responseDate}`}
                          </span>
                          <span className="mt-1 block">{report.response}</span>
                          {report.referenceNumber && (
                            <span className="mt-1 block">
                              {locale === "hi" ? "संदर्भ" : "Reference"}:{" "}
                              {report.referenceNumber}
                            </span>
                          )}
                          {report.evidence && (
                            <em className="mt-1 block font-bold not-italic text-[#7c2b21]">
                              {locale === "hi"
                                ? "प्रमाण का नोट"
                                : "Evidence note"}
                              : {report.evidence}
                  ...
                  <p className="mt-2 text-sm text-[#536059]">
                    {locale === "hi"
                      ? "अभी कोई जवाब दर्ज नहीं है।"
                      : "No portal or desk response has been recorded yet."}
                  </p>
                )}
              </div>
              <div>
                <h3 className="m-0 text-sm font-extrabold">{ui.outcomes}</h3>
                {outcomesLoadFailed ? (
                  <p
                    className="mt-2 rounded-xl bg-[#fbeceb] p-3 text-sm font-bold text-[#7c2b21]"
                    role="alert"
                  >
                    {locale === "hi"
                      ? "नतीजे अभी उपलब्ध नहीं हैं। फिर से कोशिश करें।"
                      : "Outcome evidence is unavailable. Please try again."}
                  </p>
                ) : outcomes.length > 0 ? (
                  <ol className="mt-2 grid gap-2 p-0">
                    {outcomes.map((outcome) => {
                      const step = outcome.stepId
                        ? seed.nodes.find(({ id }) => id === outcome.stepId)
                        : undefined;
                      return (
                        <li
                          className="border-l-[3px] border-[var(--marigold)] py-2 pl-4 text-sm"
                        ...
                        >
                          <strong className="block">
                            {outcomeLabels[outcome.kind]}
                          </strong>
                          <span className="mt-1 block text-[#536059]">
                            {formatTime(outcome.occurredAt)}
                          </span>
                          {step && (
                            <span className="mt-1 block">
                              {translate(step.title, locale)}
                  ...
                  <p className="mt-2 text-sm text-[#536059]">
                    {locale === "hi"
                      ? "अभी कोई नतीजा दर्ज नहीं है।"
                      : "No outcome evidence has been recorded yet."}
                  </p>
                )}
              </div>
              <div>
                <h3 className="m-0 text-sm font-extrabold">{ui.references}</h3>
                <p className="mt-2 text-sm text-[#536059]">
                  {referenceNumbers.join(" · ") || ui.noReferences}
                </p>
              </div>
            </div>
          </details>
