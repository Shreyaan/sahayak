...
export function buildActionBrief(
  ...
  if (node) {
    lines.push(
      `${t("Next action", "अगला कदम")}: ${node.title[locale]}`,
      node.detail[locale],
      node.ask[locale],
      ""
    );
    if (node.link)
      lines.push(
        node.link.action[locale],
        node.link.url,
        node.link.collect[locale],
        ""
      );
    if (node.visit)
      lines.push(
        `${t("Where to ask", "कहाँ पूछें")}: ${node.visit.office[locale]}`,
        node.visit.why[locale],
        t("Keep ready", "तैयार रखें"),
        ...node.visit.carry.map((item) => `□ ${item[locale]}`),
    ...
    lines.push(
      t(
        "No action is currently open. Keep the Case Card for your history and outcome evidence.",
        "अभी कोई कदम खुला नहीं है। इतिहास और नतीजे के प्रमाण के लिए Case Card रखें।"
      )
    );
  const references =
    snapshot.reports?.filter((report) => report.referenceNumber) ?? [];
  if (references.length)
    lines.push(
      t("Your recorded references", "आपके दर्ज संदर्भ"),
      ...references.map(
  ...
  lines.push(
    t("When you return", "लौटते समय"),
    t(
      "Write the exact response: ____________________",
      "मिला सही जवाब लिखें: ____________________"
    ),
    t(
      "Response date: __________ Reference: __________",
      "जवाब की तारीख: __________ संदर्भ: __________"
    ),
    t(
      "Evidence kept with you: ____________________",
      "अपने पास रखा प्रमाण: ____________________"
    ),
    t(
    ...
    t(
      ...
      "मार्गदर्शन के स्रोत — विशेषज्ञ सत्यापन ऐप में देखें"
    ),
